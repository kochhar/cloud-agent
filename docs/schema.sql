-- Local Cloud Agent — Postgres schema
-- Text + CHECK instead of native enums: adding a status under time pressure
-- is a one-line change rather than an ALTER TYPE.
...
CREATE INDEX sessions_thinking_idx
...
CREATE INDEX sandboxes_live_heartbeat_idx
...
CREATE INDEX tool_calls_pending_idx
...
-- ===============================================================
-- The three queries that carry the design
-- ===============================================================

-- 1. Claim the right to call the LLM. Exactly one instance wins.
--    Runs in its own short transaction; the LLM call happens after it commits.
UPDATE sessions
   SET status = 'thinking',
       thinking_since = now(),
       updated_at = now()
 WHERE id = $1
   AND status IN ('awaiting_user','executing')
RETURNING id;

-- 2. Claim a tool call for a sandbox. Exactly one sandbox wins.
UPDATE tool_calls
...
-- 3. Accept a result, but only from the current epoch.
UPDATE tool_calls
   SET status = 'done',
       result = $3, exit_code = $4, commit_sha = $5,
       completed_at = now()
 WHERE id = $1
   AND status = 'dispatched'
   AND epoch = (SELECT current_epoch FROM sessions WHERE id = $2)
RETURNING id;
